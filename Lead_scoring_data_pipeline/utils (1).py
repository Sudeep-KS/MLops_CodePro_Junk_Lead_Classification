...
def load_data_into_db(db_path, db_file_name, data_directory):
    ...
    SAMPLE USAGE
        load_data_into_db()
    '''
    datafile = f"{data_directory}{DATA_FILE}"
    print("Reading csv file: ", datafile)
    df_lead_scoring = pd.read_csv(datafile)
...
def map_city_tier(db_path, db_file_name, city_tier_mapping):
    ...
    print("Dropping city_mapped column")
...
def map_categorical_vars(db_file_name, db_path, list_platform, list_medium, list_source):
    ...
    INPUTS
        ...
        list_medium : list of all the significant medium
